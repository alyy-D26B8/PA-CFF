...
def raw2outputs(raw, z_vals, mask, white_bkgd=False):
    ...
    mask = mask.float().sum(dim=1) > 8  # should at least have 8 valid observation on the ray
...
def render_rays(ray_batch,
    ...
    pixel_mask = mask[..., 0].sum(dim=2) > 1   # [N_rays, N_samples]

    # -------- CFF: compute scene-specific features for coarse points --------
    cff_features_coarse = None
    if hasattr(model, 'cff_module') and model.cff_module is not None:
        cff_features_coarse = model.cff_module(pts)       # [N_rays, N_samples, cff_out_dim]
    # ------------------------------------------------------------------------

    raw_coarse = model.net_coarse(rgb_feat, ray_diff, mask,
                                  cff_features=cff_features_coarse)   # [N_rays, N_samples, 4]
    ...
    if N_importance > 0:
        ...
        # -------- CFF: compute scene-specific features for fine points --------
        cff_features_fine = None
        if hasattr(model, 'cff_module') and model.cff_module is not None:
            cff_features_fine = model.cff_module(pts)     # [N_rays, N_total_samples, cff_out_dim]
        # ----------------------------------------------------------------------

        raw_fine = model.net_fine(rgb_feat_sampled, ray_diff, mask,
                                  cff_features=cff_features_fine)
