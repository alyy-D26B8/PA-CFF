...
def config_parser():
    ...
    ########## CFF (Contextual Feature Field) options ##########
    parser.add_argument('--use_cff', action='store_true',
                        help='enable CFF module (only used during scene-specific fine-tuning)')
    parser.add_argument('--cff_resolutions', nargs='+', type=int, default=[32, 64],
                        help='grid resolutions for multi-resolution CFF, e.g. 32 64')
    parser.add_argument('--cff_feat_dim', type=int, default=8,
                        help='feature channels per CFF grid level')
    parser.add_argument('--cff_out_dim', type=int, default=16,
                        help='CFF output dim (must match geometry_fc output, default=16)')
    parser.add_argument('--cff_bbox_size', type=float, default=4.0,
                        help='bounding-box half-size; world-coord points in [-bbox,bbox]^3')
    parser.add_argument('--cff_gate_init', type=float, default=0.0,
                        help='initial gate value (0.0 = zero CFF contribution at start)')
    parser.add_argument('--cff_reg_weight', type=float, default=0.01,
                        help='weight for total-variation regularisation on CFF grids')
    parser.add_argument('--lrate_cff', type=float, default=1e-3,
                        help='learning rate for CFF module')

    ########## fine-tuning options ##########
    parser.add_argument('--freeze_feature_net', action='store_true',
                        help='freeze ResUNet feature extractor during fine-tuning')
    parser.add_argument('--freeze_ibrnet', action='store_true',
                        help='freeze IBRNet MLP (net_coarse & net_fine) during fine-tuning')
