...
def train(args):
    ...
    # ---------- CFF config ----------
    cff_reg_weight = getattr(args, 'cff_reg_weight', 0.0)
    ...
    while global_step < model.start_step + args.n_iters + 1:
        ...
        for train_data in train_loader:
            ...
            # -------- CFF: total-variation regularisation --------
            if model.cff_module is not None and cff_reg_weight > 0:
                cff_tv_loss = model.cff_module.compute_tv_loss()
                loss = loss + cff_reg_weight * cff_tv_loss
                scalars_to_log['cff/tv_loss'] = cff_tv_loss.item()
                # NOTE: no-gate 消融版本没有 gate 参数，不再 log gate 值
            # -----------------------------------------------------
